 ...
 Nasty.Parameters = $plugins.filter(function(p)
  ...
  //----------------------------------------------------------------------------
			...
			'<div id="RegisterForm" class="panel panel-primary" style="width:'+(Graphics.boxWidth - (Graphics.boxWidth / 3))+'px">'+
				...
				'<div class="panel-body">'+
  				...
  				'<div class="form-group">'+
    				'<input type="text" id="regUsername" name="username" placeholder="Введите Логин" class="form-control"/>'+
  				'</div>'+
  					'<div class="form-group">'+
    			'<input type="text" name="email" id="regEmail"  placeholder="Email" class="form-control"/>'+
  				'</div>'+
  					'<div class="form-group">'+
    			'<input type="password" name="password" id="regPassword" placeholder="Пароль" class="form-control"/>'+
  					'</div>'+
  					'<button id="btnSubmit"type="submit" class="btn btn-default">Подтвердить</button>'+
						'<button id ="btnCancel" type="button" class="btn btn-primary">Отмена</button>'+
				'</div>'+
			'</div>');
			...
			'<div id="LoginForm" class="panel panel-primary" style="width:'+(Graphics.boxWidth - (Graphics.boxWidth / 3))+'px">'+
				...
				'<div class="panel-body">'+
					...
					'<div class="input-group">'+
						...
						'<input type="text" class="form-control login-input" id="inputUsername" placeholder="Логин" aria-describedby="username-addon">'+
					'</div><br>'+
					'<div class="input-group">'+
						'<span class="input-group-addon" id="password-addon"><i class="fa fa-lock"></i></span>'+
						'<input type="password" class="form-control login-input" id="inputPassword" placeholder="Пароль" aria-describedby="password-addon">'+
					'</div><br>'+
					'<button id="btnConnect" class="btn btn-primary">Подключиться</button>'+
					'<button id="btnRegister" class="btn btn-default">Регистрация</button>'+
          '<button id="btnForgotPassword" class="btn btn-link btn-sm">Забыли пароль?</button>'+
				'</div>'+
			'</div>');
	...
	MMO_Scene_Title.prototype.createLogoutContinueForm = function(name, token) {
		$("#ErrorPrinter").html(
			`<div id="ActivationForm" class="panel panel-primary" style="width: ${(Graphics.boxWidth - (Graphics.boxWidth / 3))}px">
				<div class="panel-heading">Приветствую ${name}</div>
				<div class="panel-body">
					<div id="loginErrBox"></div>
					<div class="input-group">
					</div><br>
					<button id="btnContinue" class="btn btn-primary">Продолжить</button>
					<button id="btnLogout" class="btn btn-default">Выйти</button>
				</div>
			</div>`);
  ...
  MMO_Scene_Title.prototype.createLostPasswordForm = function() {
		$("#ErrorPrinter").html(
			'<div id="LostPasswordFrom" class="panel panel-primary" style="width:'+(Graphics.boxWidth - (Graphics.boxWidth / 3))+'px">'+
				'<div class="panel-heading">Забыли парол - Введите Email</div>'+
				'<div class="panel-body">'+
					'<div id="loginErrBox"></div>'+
  					'<div class="form-group">'+
    			'<input type="text" name="email" id="inputEmailLP"  placeholder="Email" class="form-control"/>'+
  				'</div>'+
  					'<button id="btnSubmitLP"type="submit" class="btn btn-primary">Подтвердить</button>'+
						'<button id ="btnCancelLP" type="button" class="btn btn-default">Отмена</button>'+
				'</div>'+
			'</div>');
    ...
    MMO_Scene_Title.prototype.createResetPasswordForm = function() {
      ...
      MMO_Scene_Title.prototype.changePasswordRequest = function(){
        ...
        if (pass1.length === 0)
    			return this.displayError("Вы должны ввести пароль!");
        if (pass2.length === 0)
      		return this.displayError("Вы должны подтвердить пароль!");
        if (pass1!==pass2)
          return this.displayError("Пароли не совпадают!");
        this.displayInfo('Подключение <i class="fa fa-spin fa-spinner"></i>');
        ...
        $.post($gameNetwork._serverURL+'/resetpassword', {
            ...
            that.createLoginForm();
            that.displaySuccess("Пароль успешно изменён");
          });
      };
    ...
    MMO_Scene_Title.prototype.lostPasswordRequest = function(){
      ...
      if (email.length === 0)
  			return this.displayError("Вы должны ввести логин!");
      this.displayInfo('Подключение <i class="fa fa-spin fa-spinner"></i>');

      $.post($gameNetwork._serverURL+'/lostpassword', {
        email: email,
      }).done(function (data) {
        if (data.err) return that.displayError("Ошибка : "+data.err);
        that.createLoginForm();
	...
	MMO_Scene_Title.prototype.registerAttempt = function(){
		...
		if (username.length === 0) return this.displayError("Вы должны ввести имя!");
		if (password.length === 0) return this.displayError("Вы должны ввести пароль!");
		if (email.length === 0) return this.displayError("Вы должны ввести Email!");
		...
		if (data.msg) {
			...
			that.displayInfo("Проверьте свой email на активационную ссылку!");
		}
	});
	};
